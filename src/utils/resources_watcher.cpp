#include "resources_watcher.hpp"

ResourcesWatcher::ResourcesWatcher(QObject *parent)
    : QObject(parent)
    , m_prevTotal(0)
    , m_prevIdle(0)
{
...
QVariantMap ResourcesWatcher::makeGpuEntry(const QString &vendor, int index, const QString &name)
{
    return QVariantMap{
...
void ResourcesWatcher::updateDiskStats()
{
    QVariantList disks;
...
void ResourcesWatcher::updateStats()
{
...
void ResourcesWatcher::readCpuStats(double &cpuPercent, double &cpuFreqGhz, double &cpuTotalFreqGhz, double &cpuTemp)
{
...
void ResourcesWatcher::readMemoryAndSwapStats(quint64 &memTotal, quint64 &memAvailable, quint64 &swapTotal, quint64 &swapFree)
{
    struct sysinfo info;
    if (sysinfo(&info) == 0) {
...
QVariantList ResourcesWatcher::readGpuStats()
{
    QVariantList gpus;
...
void ResourcesWatcher::readNvidiaGpuStats(QVariantList &gpus)
{
    ...
    if (nvmlDeviceGetCount(&deviceCount) == NVML_SUCCESS) {
        for (unsigned int i = 0; i < deviceCount; ++i) {
            ...
            tempInfo.sensorType = NVML_TEMPERATURE_GPU; // Set the sensor type in the struct
...
void ResourcesWatcher::readAmdGpuStats(QVariantList &gpus)
{
...
void ResourcesWatcher::readIntelGpuStats(QVariantList &gpus)
{
    QDir driDir("/dev/dri");
